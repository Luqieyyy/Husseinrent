...
export default function StudentChatWidget({ userId }: StudentChatWidgetProps) {
  ...
  const loadConversations = async () => {
    ...
    if (result.conversations) {
      ...
      // Calculate unread count (placeholder - can add unread tracking later)
    ...
    <>
            ...
            <div className="flex-1 overflow-y-auto">
              {conversations.length === 0 ? (
                <div className="p-8 text-center text-gray-400">
                  ...
                  <p>No messages yet</p>
                  <p className="text-sm mt-2">Chat with a landlord from property details.</p>
                </div>
              ) : (
                conversations.map((conv) => (
                  ...
                  >
                    <div className="flex justify-between items-start mb-1">
                      <p className="font-semibold text-white">{conv.landlord_name}</p>
      ...
      {selectedConversation && (
        <ChatWindow
          ...
          userId={userId}
          otherUserId={selectedConversation.landlord_id}
          otherUserName={selectedConversation.landlord_name}
